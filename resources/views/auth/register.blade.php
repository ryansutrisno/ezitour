...
@section('title', __('front.register_title') . ' - EziTour')
...
<div class="min-h-[calc(100vh-4rem)] grid lg:grid-cols-2">
    ...
    <div class="relative hidden lg:flex flex-col justify-between overflow-hidden bg-gradient-to-br from-blue-700 via-blue-700 to-blue-800 p-12 text-white">
        ...
        <div class="relative">
            <h2 class="font-display text-4xl font-extrabold leading-tight">{{ __('front.register_brand_title') }}<br>{{ __('front.register_brand_title_2') }}</h2>
            <p class="mt-4 text-blue-100 max-w-md leading-relaxed">{{ __('front.register_brand_body') }}</p>
            ...
            <ul class="mt-8 space-y-3.5">
                <li class="flex items-center gap-3 text-blue-50">
                    ...
                    <span class="text-sm">{{ __('front.register_brand_bullet_1') }}</span>
                </li>
                <li class="flex items-center gap-3 text-blue-50">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-white/15"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></span>
                    <span class="text-sm">{{ __('front.register_brand_bullet_2') }}</span>
                </li>
                <li class="flex items-center gap-3 text-blue-50">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-white/15"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></span>
                    <span class="text-sm">{{ __('front.register_brand_bullet_3') }}</span>
                </li>
            </ul>
        </div>

        <p class="relative text-sm text-blue-200">&copy; {{ date('Y') }} EziTour. {{ __('front.footer_tagline_brand') }}</p>
    </div>
        ...
        <div class="w-full max-w-md mx-auto">
            ...
            <div class="bg-white rounded-card border border-slate-100 shadow-card overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5">
                    <h1 class="font-display text-xl font-bold text-white">{{ __('front.register_form_title') }}</h1>
                    <p class="mt-1 text-sm text-blue-100">{{ __('front.register_form_subtitle') }}</p>
                </div>
                ...
                <div class="p-6 sm:p-7">
                    ...
                    <form class="space-y-5" action="{{ route('register.store') }}" method="POST">
                        ...
                        {{-- Name --}}
                        <div>
                            <label for="name" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('front.register_name_label') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                </div>
                                <input id="name" name="name" type="text" autocomplete="name" required value="{{ old('name') }}" placeholder="{{ __('front.register_name_placeholder') }}" class="block w-full pl-11 pr-3.5 py-3 rounded-input border-2 border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:bg-white transition-colors sm:text-sm @error('name') border-red-400 bg-red-50 @enderror">
                            </div>
                            @error('name')
                        ...
                        {{-- Password --}}
                        <div>
                            <label for="password" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('front.register_password_label') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                ...
                                <input id="password" name="password" type="password" autocomplete="new-password" required placeholder="{{ __('front.register_password_placeholder') }}" class="block w-full pl-11 pr-11 py-3 rounded-input border-2 border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:bg-white transition-colors sm:text-sm @error('password') border-red-400 bg-red-50 @enderror">
                                <button type="button" onclick="togglePasswordVisibility('password')" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 focus:outline-none cursor-pointer" aria-label="{{ __('front.button_show_password') }}">
                        ...
                        <div>
                            <label for="password_confirmation" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('front.register_password_confirm_label') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                                </div>
                                <input id="password_confirmation" name="password_confirmation" type="password" autocomplete="new-password" required placeholder="{{ __('front.register_password_confirm_placeholder') }}" class="block w-full pl-11 pr-11 py-3 rounded-input border-2 border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 focus:border-blue-500 focus:ring-blue-500 focus:outline-none focus:bg-white transition-colors sm:text-sm">
                                <button type="button" onclick="togglePasswordVisibility('password_confirmation')" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 focus:outline-none cursor-pointer" aria-label="{{ __('front.button_show_password') }}">
                    ...
                    <div class="mt-5 flex items-center justify-center gap-2 text-xs text-slate-400">
                        ...
                        {{ __('front.register_trust') }}
                    </div>
                </div>
            </div>

            <p class="mt-6 text-center text-sm text-slate-600">
                {{ __('front.register_login_prompt') }}
                <a href="{{ route('login') }}" class="font-semibold text-blue-600 hover:text-blue-700">{{ __('front.register_login_link') }}</a>
            </p>
        </div>
    </div>
</div>
