...
@section('title', __('front.login_title') . ' - EziTour')
...
<div class="min-h-[calc(100vh-4rem)] grid lg:grid-cols-2">
    ...
    <div class="relative hidden lg:flex flex-col justify-between overflow-hidden bg-gradient-to-br from-blue-700 via-blue-700 to-blue-800 p-12 text-white">
        ...
        <div class="relative">
            <h2 class="font-display text-4xl font-extrabold leading-tight">{{ __('front.login_brand_title') }}<br>{{ __('front.login_brand_title_2') }}</h2>
            <p class="mt-4 text-blue-100 max-w-md leading-relaxed">{{ __('front.login_brand_body') }}</p>
            ...
            <ul class="mt-8 space-y-3.5">
                <li class="flex items-center gap-3 text-blue-50">
                    ...
                    <span class="text-sm">{{ __('front.login_brand_bullet_1') }}</span>
                </li>
                <li class="flex items-center gap-3 text-blue-50">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-white/15"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></span>
                    <span class="text-sm">{{ __('front.login_brand_bullet_2') }}</span>
                </li>
                <li class="flex items-center gap-3 text-blue-50">
                    <span class="flex items-center justify-center w-7 h-7 rounded-full bg-white/15"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></span>
                    <span class="text-sm">{{ __('front.login_brand_bullet_3') }}</span>
                </li>
            </ul>
        </div>
        ...
        <div class="w-full max-w-md mx-auto">
            ...
            <div class="bg-white rounded-card border border-slate-100 shadow-card overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5">
                    <h1 class="font-display text-xl font-bold text-white">{{ __('front.login_form_title') }}</h1>
                    <p class="mt-1 text-sm text-blue-100">{{ __('front.login_form_subtitle') }}</p>
                </div>

                <div class="p-6 sm:p-7">
                    {{-- Session / generic errors --}}
                    @if (session('error'))
                    ...
                    <form class="space-y-5" action="{{ route('login.store') }}" method="POST">
                        ...
                        {{-- Email --}}
                        <div>
                            <label for="email" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('front.login_email_label') }}</label>
                            <div class="relative">
                        ...
                        {{-- Password --}}
                        <div>
                            <label for="password" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('front.login_password_label') }}</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
            ...
            <p class="mt-6 text-center text-sm text-slate-600">
                {{ __('front.login_register_prompt') }}
                <a href="{{ route('register') }}" class="font-semibold text-blue-600 hover:text-blue-700">{{ __('front.login_register_link') }}</a>
            </p>
        </div>
    </div>
</div>
