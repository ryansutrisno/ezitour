...
@section('title', 'Wishlist Saya - EziTour')

@section('seo')
    <meta name="robots" content="noindex, nofollow">
    <meta name="description" content="Paket wisata favorit yang kamu simpan di EziTour.">
@endsection
        ...
        <div class="relative max-w-7xl mx-auto py-14 sm:py-20 px-4 sm:px-6 lg:px-8">
            <nav class="mb-4" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1.5 text-xs">
                    <li><a href="{{ route('front.home') }}" class="text-blue-100 hover:text-white transition-colors">Beranda</a></li>
                    <li class="text-blue-200">/</li>
                    <li class="text-white font-semibold" aria-current="page">Wishlist</li>
                </ol>
            </nav>
            <h1 class="font-display text-4xl sm:text-5xl font-extrabold tracking-tight text-white">Wishlist Saya</h1>
            <p class="mt-4 max-w-2xl text-lg text-blue-100 leading-relaxed">Paket wisata favorit yang kamu simpan.</p>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        @if($packages->isNotEmpty())
            <div class="flex items-center justify-between mb-6">
                <p class="text-sm text-slate-500">
                    Menampilkan
                    <span class="font-semibold text-slate-700">{{ $packages->firstItem() }}</span>–
                    <span class="font-semibold text-slate-700">{{ $packages->lastItem() }}</span>
                    dari
                    <span class="font-semibold text-slate-700">{{ $packages->total() }}</span>
                    paket tersimpan
                </p>
            </div>
        ...
        @else
            ...
            <div class="bg-white rounded-card border border-slate-100 shadow-card p-12 sm:p-16 text-center">
                ...
                <h3 class="mt-6 font-display text-xl font-bold text-slate-900">Belum ada paket tersimpan</h3>
                <p class="mt-2 text-sm text-slate-500 max-w-sm mx-auto">Cari paket favoritmu dan simpan untuk akses cepat.</p>
                <div class="mt-7">
