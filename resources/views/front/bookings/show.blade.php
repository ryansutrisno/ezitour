    ...
    <div class="min-h-[calc(100vh-4rem)] bg-slate-50 py-8 sm:py-12">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="mb-6 flex items-center gap-2 text-sm text-slate-500 flex-wrap">
                <a href="{{ route('front.home') }}" class="hover:text-blue-600 transition-colors">Beranda</a>
                ...
                <svg class="w-3.5 h-3.5 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                <span class="text-slate-700 font-medium">Detail Booking #{{ $booking->code ?? $booking->id }}</span>
            </nav>
            ...
            <div class="mb-6 relative overflow-hidden rounded-card bg-gradient-to-br from-blue-700 via-blue-700 to-blue-800 shadow-card">
                ...
                <div class="relative p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <p class="text-blue-200 text-xs font-semibold uppercase tracking-wider">Kode Booking</p>
                    ...
                    <div>
                        @php
                            $statusConfig = [
                                'pending' => ['Menunggu Pembayaran', 'bg-yellow-400 text-yellow-900'],
                                'paid' => ['Lunas', 'bg-green-400 text-green-900'],
                                'cancelled' => ['Dibatalkan', 'bg-red-400 text-red-900'],
                                'completed' => ['Selesai', 'bg-blue-400 text-white'],
                            ];
                            [$statusLabel, $statusClass] = $statusConfig[$booking->status] ?? ['Unknown', 'bg-slate-400 text-white'];
                        @endphp
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-card border border-slate-100 shadow-card p-6">
                        <h2 class="font-display text-lg font-bold text-slate-900 mb-4">Detail Paket</h2>
                    ...
                    {{-- Traveler info --}}
                    <div class="bg-white rounded-card border border-slate-100 shadow-card p-6">
                        <h2 class="font-display text-lg font-bold text-slate-900 mb-4">Data Traveler</h2>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div>
                                <p class="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Nama</p>
                                <p class="mt-1 font-semibold text-slate-800">{{ $booking->user->name }}</p>
                            </div>
                            <div>
                                <p class="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Email</p>
                                <p class="mt-1 font-semibold text-slate-800 break-all">{{ $booking->user->email }}</p>
                            </div>
                            @if($booking->user->phone)
                            <div>
                                <p class="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Telepon</p>
                    ...
                    {{-- Payment history --}}
                    <div class="bg-white rounded-card border border-slate-100 shadow-card p-6">
                        <h2 class="font-display text-lg font-bold text-slate-900 mb-4">Riwayat Pembayaran</h2>
                        @if($booking->transactions->isEmpty())
                            <p class="text-sm text-slate-500 py-4 text-center">Belum ada transaksi pembayaran.</p>
                        @else
                            <div class="space-y-3">
                                @foreach($booking->transactions as $txn)
                                    @php
                                        $txnStatusConfig = [
                                            ...
                                            'superseded' => ['Digantikan', 'bg-slate-100 text-slate-500'],
                                        ];
                                    ...
                                    <div class="flex items-center justify-between gap-3 p-3.5 rounded-button bg-slate-50 border border-slate-100">
                                        <div class="min-w-0">
                                            ...
                                            <p class="mt-0.5 text-sm text-slate-700">
                                                {{ $txn->payment_type ? ucwords(str_replace('_', ' ', $txn->payment_type)) : 'Menunggu metode' }}
                ...
                <div class="lg:col-span-1">
                    <div class="lg:sticky lg:top-20 space-y-4">
                        ...
                        <div class="bg-white rounded-card border border-slate-100 shadow-card overflow-hidden">
                            <div class="p-6">
                                <h2 class="font-display text-lg font-bold text-slate-900 mb-4">Ringkasan</h2>
                                <div class="space-y-3 text-sm">
                                    ...
                                    @if($booking->hasDiscount() || $booking->hasCouponDiscount())
                                        ...
                                        @if($booking->hasDiscount())
                                            <div class="flex justify-between">
                                                <span class="text-green-600">Diskon Rombongan{{ $booking->applied_tier_label ? ' ('.$booking->applied_tier_label.')' : '' }}</span>
                                                <span class="font-bold text-green-600">- Rp {{ number_format((float) $booking->discount_amount, 0, ',', '.') }}</span>
                                            </div>
                                        @endif
                                        @if($booking->hasCouponDiscount())
                                            <div class="flex justify-between">
                                                <span class="text-green-600">Diskon Promo{{ $booking->coupon_code ? ' ('.$booking->coupon_code.')' : '' }}</span>
                        ...
                        <div class="bg-white rounded-card border border-slate-100 shadow-card p-6">
                            @if($booking->status === 'pending')
                                ...
                                <form method="POST" action="{{ route('bookings.cancel', $booking) }}" onsubmit="return confirm('Yakin ingin membatalkan booking ini? Tindakan ini tidak dapat dibatalkan.');">
                                    @csrf
                            ...
                            @elseif($booking->status === 'cancelled')
                                <div class="p-4 rounded-button bg-red-50 border border-red-100 text-center mb-3">
                                    ...
                                    <p class="text-sm text-red-700 font-semibold">Booking ini telah dibatalkan.</p>
                                </div>
                        ...
                        <a href="{{ route('dashboard.index') }}" class="block text-center text-sm text-slate-500 hover:text-blue-600 transition-colors py-2">&larr; Kembali ke Dashboard</a>
                    </div>
                </div>
            </div>
        </div>
