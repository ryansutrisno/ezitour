...
<div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
        <h2 class="text-xl font-bold text-white flex items-center">
            ...
            {{ __('packages.checkout_booking_form_title') }}
        </h2>
    </div>
    ...
    <div class="p-6">
        <form id="booking-form" action="{{ route('front.checkout.store', $package->slug) }}" method="POST">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <label for="travel_date" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('packages.checkout_travel_date_label') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                ...
                <div>
                    <label for="participants" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('packages.checkout_participants_label') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    ...
                    @error('participants')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-2 text-xs text-gray-500">{{ __('packages.checkout_participants_hint') }}</p>
                </div>

                <!-- Pickup Location -->
                <div>
                    <label for="pickup_location" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('packages.checkout_pickup_label') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        ...
                        <textarea
                            ...
                            required
                            placeholder="{{ __('packages.checkout_pickup_placeholder') }}"
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border border-blue-100">
                    <h4 class="text-sm font-bold text-blue-900 mb-4 flex items-center">
                        ...
                        {{ __('packages.checkout_price_breakdown_title') }}
                    </h4>
                    <div class="space-y-3 text-sm">
                        <div class="flex justify-between items-center text-blue-800">
                            <span>{{ __('packages.checkout_price_per_pax') }}</span>
                            <span class="font-semibold" data-role="per-pax-price">Rp {{ number_format($package->total_price, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between items-center text-blue-800">
                            <span>{{ __('packages.checkout_participants_count') }}</span>
                        ...
                        {{-- Discount badge (hidden by default, toggled via JS) --}}
                        <div id="discount-badge" class="hidden text-xs font-semibold text-green-700 bg-green-100 rounded-lg px-3 py-2 flex items-center gap-1.5">
                            <svg class="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5 2a1 1 0 011 1v1h1a1 1 0 010 2H6v1a1 1 0 01-2 0V6H3a1 1 0 010-2h1V3a1 1 0 011-1zm0 10a1 1 0 011 1v1h1a1 1 0 110 2H6v1a1 1 0 11-2 0v-1H3a1 1 0 110-2h1v-1a1 1 0 011-1zM12 2a1 1 0 01.967.744L14.146 7.2 17.5 9.134a1 1 0 010 1.732l-3.354 1.935-1.18 4.455a1 1 0 01-1.933 0L9.854 12.8 6.5 10.866a1 1 0 010-1.732L9.854 7.2l1.18-4.455A1 1 0 0112 2z" clip-rule="evenodd"/></svg>
                            <span data-role="discount-text"></span>
                        </div>
                        <div class="border-t border-blue-200 pt-3 mt-3">
                            <div class="flex justify-between items-center">
                                <span class="font-bold text-blue-900 text-base">{{ __('packages.checkout_total') }}</span>
            ...
            @if($isAuthenticated)
                ...
                >
                    {{ __('packages.checkout_continue_payment') }}
                </button>
            @else
                <button
                    type="submit"
                    class="mt-6 w-full flex justify-center items-center py-4 px-6 border border-transparent rounded-xl shadow-lg text-base font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200"
                >
                    {{ __('packages.checkout_continue_auth') }}
                </button>
                <p class="mt-3 text-xs text-center text-gray-500">
                    {{ __('packages.checkout_guest_hint') }}
                </p>
            @endif
        </form>
    </div>
