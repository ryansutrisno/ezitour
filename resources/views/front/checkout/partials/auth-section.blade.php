...
<div id="auth-section" class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden {{ !$showAuth ? 'hidden' : '' }}">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
        <h2 class="text-xl font-bold text-white flex items-center">
            ...
            {{ __('packages.checkout_auth_section_title') }}
        </h2>
    </div>
    ...
    <div class="p-6">
        ...
        <div class="bg-gray-100 p-1 rounded-xl mb-6">
            <nav class="flex" aria-label="Tabs">
                ...
                >
                    {{ __('packages.checkout_auth_login_tab') }}
                </button>
                <button
                    type="button"
                ...
                >
                    {{ __('packages.checkout_auth_register_tab') }}
                </button>
            </nav>
        </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="login_email" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('front.label_email') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                ...
                <div>
                    <label for="login_password" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('front.label_password') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <input
                            ...
                            required
                            placeholder="{{ __('front.login_password_placeholder') }}"
                        >
    ...
    <div id="register-form-container" class="{{ $activeTab !== 'register' ? 'hidden' : '' }}">
        <form action="{{ route('front.checkout.register', $package->slug) }}" method="POST">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="register_name" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('front.register_name_label') }} <span class="text-red-500">*</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <input
                            ...
                            required
                            value="{{ old('name') }}"
                            placeholder="{{ __('front.register_name_placeholder') }}"
                        >
                    </div>
                    @error('register_name')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    <label for="register_phone" class="block text-sm font-semibold text-gray-700 mb-2">
                        {{ __('front.label_phone') }} <span class="text-gray-400 text-xs font-normal">{{ __('front.label_optional') }}</span>
                    </label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input
                                ...
                                required
                                placeholder="{{ __('front.register_password_placeholder') }}"
                            >
                    ...
                    <div>
                        <label for="register_password_confirmation" class="block text-sm font-semibold text-gray-700 mb-2">
                            {{ __('front.register_password_confirm_label') }} <span class="text-red-500">*</span>
                        </label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <input
                                ...
                                class="block w-full pl-10 pr-10 rounded-xl border-2 border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm bg-gray-50 p-3.5 transition-colors hover:border-gray-300"
                                required
                                placeholder="{{ __('front.register_password_confirm_placeholder') }}"
                            >
                        ...
                        <p id="password-match-error" class="mt-2 text-sm text-red-600 hidden">{{ __('packages.checkout_password_mismatch') }}</p>
                    </div>
                </div>
            </div>
            ...
            >
                {{ __('packages.checkout_auth_register_submit') }}
            </button>
        </form>

        <div class="mt-6 text-center">
            <p class="text-sm text-gray-500">
                {{ __('packages.checkout_auth_register_prompt') }}
                <button type="button" onclick="switchTab('login')" class="text-blue-600 hover:text-blue-700 font-semibold hover:underline">
                    {{ __('packages.checkout_auth_register_link') }}
                </button>
            </p>
        </div>
    </div>
